	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!Security.isActiveSession(request, response)) {
			...
			if (request.getParameter("nameOfUserSelect") != null
					&& !request.getParameter("nameOfUserSelect").equals("Choose User")) {
				...
				try {
					UserOrdersDto userOrdersDto = IocContainer.get().getUserOrdersServise()
							.getUserOrders(IocContainer.get().getUserService()
				...
				request.setAttribute("redirectURLInOrderCount",
						RedirectURLInOrderCount.ORDERS_USERS_ADMIN_SERVLET.toString());
				getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.ORDERS_USERS_ADMIN_JSP.toString())
						.forward(request, response);
			} else {
				getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.ORDERS_USERS_ADMIN_JSP.toString())
						.forward(request, response);
