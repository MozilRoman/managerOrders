...
public class OrderProfileEditOrderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!Security.isActiveSession(request, response)) {
			...
			OrderDto orderDto = orderServise.geOrderDto(Long.parseLong(request.getParameter("idOrder")));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getSession(false).getAttribute("idUserAddItem") != null
			...
			if (Integer.parseInt(request.getParameter("scope")) >= 1) {
				...
				orderDto = new OrderDto(Long.parseLong(request.getParameter("idOrder")), request.getParameter("shop"),
						request.getParameter("address"), request.getParameter("production"),
						Integer.parseInt(request.getParameter("scope")), request.getParameter("status"));
				orderServise.setOrderDto(orderDto, idUserAddItem);

				request.setAttribute("changesSavedSuccessfully", "Changes saved successfully");

				response.sendRedirect(request.getContextPath() + ControllerUrls.ORDERS_SERVLET.toString());
			} else {
				orderDto = new OrderDto(Long.parseLong(request.getParameter("idOrder")), request.getParameter("shop"),
						request.getParameter("address"), request.getParameter("production"),
						Integer.parseInt(request.getParameter("scope")), request.getParameter("status"));
			...
			if (Integer.parseInt(request.getParameter("scope")) >= 1) {
				orderDto = new OrderDto(Long.parseLong(request.getParameter("idOrder")), request.getParameter("shop"),
						request.getParameter("address"), request.getParameter("production"),
						Integer.parseInt(request.getParameter("scope")), request.getParameter("status"));
				orderServise.setOrderDto(orderDto,
