...
public class Orders extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!Security.isActiveSession(request, response)) {
			...
			try {
				userOrdersDto = IocContainer.get().getUserOrdersServise()
				...
				userOrdersDto = new UserOrdersDto(
						((LoginDto) request.getSession(true).getAttribute("loginDto")).getLogin());
			...
			getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.ORDERS_JSP.toString()).forward(request,
					response);
		}
	}
