...
public class UsersProfilesEditAdminServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!Security.isActiveSession(request, response)) {
			...
			if (request.getParameter("nameOfUserSelect") != null
					&& !request.getParameter("nameOfUserSelect").equals("Choose User")) {// the data send in get
																							// method(in URL line) check
																							// if User doesn`t edit
																							// nameOfUserSelect; check
																							// if this name exist in DB
				...
				getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.EDIT_PROFILES_USERS_JSP.toString())
						.forward(request, response);
			} else {
				getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.EDIT_PROFILES_USERS_JSP.toString())
						.forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("newRole") != null && request.getParameter("newRole").equals("Administrator")
			...
			response.sendRedirect(
					request.getContextPath() + ControllerUrls.USERS_PROFILES_EDIT_ADMIN_SERVLET.toString());

		} else if (request.getParameter("newRole") != null && request.getParameter("newRole").equals("User")
				&& !request.getParameter("newRole").equals(request.getParameter("currentRole"))) {
