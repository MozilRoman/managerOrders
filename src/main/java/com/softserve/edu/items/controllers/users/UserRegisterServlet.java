...
public class UserRegisterServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.REGISTER_JSP.toString()).forward(request,
				response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		UserDto userDto = new UserDto(request.getParameter("login"), request.getParameter("password"),
		...
		if (!userService.isUserExist(userDto.getLogin())) {
			if (request.getParameter("password").equals(request.getParameter("password_repeat"))) {
				...
				getServletConfig().getServletContext().getRequestDispatcher(ViewUrls.REGISTER_JSP.toString())
						.forward(request, response);
			}
		} else {
